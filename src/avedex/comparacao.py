...
def comparar_aves(aves):
    ...
    ave_1 = escolher_ave(aves, "Digite o ID da primeira ave")
    if ave_1 is None:
        return

    ave_2 = escolher_ave(aves, "Digite o ID da segunda ave")
    if ave_2 is None:
        return

    titulo("COMPARAÇÃO LADO A LADO")
    imprimir_linha("Campo", ave_1["nome_popular"], ave_2["nome_popular"])
    print(linha("-", 78))

    for rotulo, campo, unidade in CAMPOS_COMPARACAO:
        valor_1 = preparar_valor(ave_1, campo, unidade)
        valor_2 = preparar_valor(ave_2, campo, unidade)
        imprimir_linha(rotulo, valor_1, valor_2)
